...
# print('Count objects before filter null data',movies_df.count())
# filter_movies_df = movies_df.filter('Title is not null')
# print('Count objects after filter nulll data',filter_movies_df.count())
...
most_popular_df = ratings_df\
    .groupBy("MovieID")\
    .agg(count("UserID"))\
    .withColumnRenamed("count(UserID)", "Viewer Count")\
    .sort(desc("Viewer Count"))
...
top_rated_movie_df = ratings_df\
    .groupBy("MovieID")\
    .agg(avg(col("Rating")))\
    .withColumnRenamed("avg(Rating)","Average Viewer Rating")\
    .sort(desc("Average Viewer Rating"))
...
popular_rated_df = most_popular_movies_df\
    ...
    .drop(top_rated_movie_df.MovieID)\
    .withColumn('Average Viewer Rating', func.round(top_rated_movie_df['Average Viewer Rating'],2))\
    .sort(desc("Average Viewer Rating"))
